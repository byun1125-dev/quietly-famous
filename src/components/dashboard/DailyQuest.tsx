...
export default function DailyQuest() {
  const [completed, setCompleted] = useSyncData<string[]>("completed_missions", []);
  ...
  const done = completed.filter(id => activeMissions.some(m => m.id === id)).length;
    ...
    <div className="card-minimal">
      <div className="flex justify-between items-end mb-10">
        ...
        <div className="text-right">
          ...
          <p className="font-bold">{done === total ? "ALL CLEAR" : "IN PROGRESS"}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {activeMissions.map((mission) => {
          ...
          return (
            <button
              key={mission.id}
              onClick={() => setCompleted(prev => isDone ? prev.filter(id => id !== mission.id) : [...prev, mission.id])}
